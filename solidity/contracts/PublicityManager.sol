...
interface IPublicityManager {
  ...
  function createPublicity(
    ...
    uint64 fee
  ) external returns (uint256 publicityId);

  function buyoutPublicity(
    uint256 publicityId,
    string calldata newText,
    string calldata newImg
  ) external payable;
...
contract PublicityManager is IPublicityManager {
  ...
  function createPublicity(
    ...
    _publicity[publicityId] = Publicity({text: text, img: img, lastPrice: initialPrice, fee: fee, owner: msg.sender, lastPublisher: address(0)});
  }
